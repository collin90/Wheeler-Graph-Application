...
def print_order(filename, MAX_ITERATIONS=2**20):
    """Returns the nodes from the ordering so that we can see the solution."""
    infile = open(test_folder + filename, 'r')
    graph_string = infile.readline().removesuffix('\n')
    infile.close()
    print(find_ordering(json.loads(graph_string), MAX_ITERATIONS)['ordering']['nodes'])

# depends on getSingleStringWheelerGraph
class TestIsWheeler(unittest.TestCase):
    def test_is_wheeler(self):
        ...
        self.assertEqual(ALL_ORDERS_MESSAGE, get_message('simple_false1.txt')) # could check for a cycle with all the same edge labels
        ...
        self.assertEqual(GOOD_MESSAGE, get_message('complex_orderable2.txt'))
        ...
        self.assertEqual(ALL_ORDERS_MESSAGE, get_message('complex_unorderable1.txt', 2**23)) # 12n, 15e => 113 billion worst case. This could be detected by finding a cycle
