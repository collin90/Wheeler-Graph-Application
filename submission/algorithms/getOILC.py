#used in visulalize tab and the interactiveGraphWithOILC component. In both cases, lists of nodes and edges are passed, and the OILC representation is returned.
...
def getOILC (nodes, edges): 
    ...
    for n in nodes:
        if('order' in n): node_order_dict[str(n['order'])] = n['id']
        else : node_order_dict[str(n['id'])] = n['id']
